...
var movieNames=filims.map(m=>m.title)
...
//movie names having  genre action
var movieGenreActio=filims.filter(m=>m.genres.includes("Action")).map(m=>m.title);
// console.log(movieGenreActio);
//or
// filims.filter(m=>m.genres.includes("Action")).forEach(f=>console.log(f.title))



//movie names having action or drama
var movieGenreActionOrDrama=filims.filter(m=>m.genres.includes("Action") || (m.genres.includes("Drama"))).map(m=>m.title);
// console.log(movieGenreActionOrDrama);
//or
filims.filter(m=>m.genres.includes("Action")||(m.genres.includes("Drama"))).forEach(m=>console.log(m.title))



//movie name having higher runtime
var movieHigherRuntime=filims.reduce((m1,m2)=>Number(m1.runtime)>Number(m2.runtime)?m1:m2);
// console.log(movieHigherRuntime);
//or
// var movieHigherRuntime=filims.reduce((m1,m2)=>+m1.runtime>+m2.runtime?m1:m2); // put + sign infront of the string where you need to change to number
// console.log(movieHigherRuntime);



//movie name having lower runtime
var movieLowerRuntime=filims.reduce((f1,f2)=> +f1.runtime>+f2.runtime?f2:f1)
// console.log(movieLowerRuntime);



//in which year, moviees released the most
var filmyears=filims.map(f=>f.year)
var wc={}
filmyears.map(y=>y in wc?wc[y]+=1:wc[y]=1)
var data=Object.entries(wc).sort((o1,o2)=>o2[1]-o1[1])
// console.log(data);
// var mostmovieyear=Object.entries(wc).reduce((o1,o2)=>o1[1]>o2[1]?o1:o2)


//which genre have most movies
var genre=filims.map(f=>f.genres).flat()
var wc={}
var genreCount=genre.map(g=>g in wc?wc[g]+=1:wc[g]=1)
console.log(wc);
var data=Object.entries(wc).sort((o1,o2)=>o2[1]-o1[1]).slice(0,6)
console.log(data);



//most movies having directors
var allDirectors=filims.map(f=>f.director.split(",")).flat()

var wc={}
var data=allDirectors.map(d=>d in wc?wc[d]+=1:wc[d]=1)
var datase=Object.entries(wc).sort((o1,o2)=>o2[1]-o1[1])
console.log(datase);